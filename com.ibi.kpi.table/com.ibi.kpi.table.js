...
(function () {
    ...
    // gets the event handler of a series
    function getDataEvent(series, chart) {
        if (typeof chart !== "undefined " && typeof chart.eventDispatcher !== "undefined" && typeof chart.eventDispatcher.events !== "undefined") {
            for (var x = 0, len = chart.eventDispatcher.events.length; x < len; x++) {
                if (chart.eventDispatcher.events[x].series === series) {
                    return getEventFuncObject(chart.eventDispatcher.events[x]);
                }
            }
        }
    }

    // gets an event handler from the charts event dispatcher event object
    function getEventFuncObject(eventObj) {
        var funcReturn = function () { };

        // Probably should be using something within chart.js so don't have to rewrite the handling
        if (eventObj.event === "setURL") {
            funcReturn = function () {
                window.open(eventObj.url, eventObj.target);
            };
        }
    ...
    function renderCallback(renderConfig) {
        ...
        if (typeof data[0].keymeasure !== 'undefined' && typeof data[0].measure !== 'undefined') {
            ...
            data.forEach(function (row, index) {
                ...
                // add the event handler to the slide
                var event = getDataEvent(index, chart);
                ...
                for (var i = 0; i < numOfMeasures; i++) {
                    if (numOfMeasures == 1) {
                        $(container)
							...
							.append('<td class="kpi-table-title kpi-title" style="font-weight:' + props.columnTitle.fontWeight
                    ...
                    var riserClass = chart.buildClassName("riser", 0, i, "cell");

                    $(container)
						.find('.kpi-table-container>.kpi-table-slide:last tbody>tr:last')
						.append('<td class="kpi-table-data kpi-value ' + riserClass + '" style="font-weight:' + props.columnData.fontWeight
